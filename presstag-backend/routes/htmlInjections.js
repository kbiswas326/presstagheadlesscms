  ...
  return LayoutConfig.create(
    {
      ...
      seo: {},
      adsTxt: '',
      htmlInjections: { head: '', headEnd: '', bodyStart: '', bodyEnd: '' },
    },
    tenantId
  );
};
...
router.get('/', async (req, res) => {
  try {
    const cfg = await ensureLayoutConfig(req.tenantId);
    const injections = cfg?.htmlInjections || {};
    res.json({
      htmlInjections: {
        head: String(injections.head || ''),
        headEnd: String(injections.headEnd || ''),
        bodyStart: String(injections.bodyStart || ''),
        bodyEnd: String(injections.bodyEnd || ''),
      },
    });
  } catch (error) {
...
router.put('/', auth, async (req, res) => {
  try {
    ...
    const next = {
      head: String(incoming?.head || ''),
      headEnd: String(incoming?.headEnd || ''),
      bodyStart: String(incoming?.bodyStart || ''),
      bodyEnd: String(incoming?.bodyEnd || ''),
    };
    const updated = await LayoutConfig.update({ htmlInjections: next }, req.tenantId);
    const doc = updated?.value || updated;
    const saved = doc?.htmlInjections || next;
    res.json({
      htmlInjections: {
        head: String(saved?.head || ''),
        headEnd: String(saved?.headEnd || ''),
        bodyStart: String(saved?.bodyStart || ''),
        bodyEnd: String(saved?.bodyEnd || ''),
      },
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
