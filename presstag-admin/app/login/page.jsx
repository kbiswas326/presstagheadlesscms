/// app/login/page.jsx | This page renders the login form for the PressTag admin dashboard. It includes fields for email and password, with validation and error handling. The page also features a "Forgot Password" link that opens a modal for users to request a password reset email. The component uses React state to manage form inputs, loading states, and error messages, and it communicates with the backend API for authentication and password reset requests. The design is responsive and includes visual feedback for user interactions.
"use client";
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <button type="button" onClick={openForgotPassword} className="text-sm text-blue-600 hover:text-blue-700 font-medium">
                  Forgot password?
                </button>
              </div>
              <div className="relative">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-medium rounded-lg shadow-md shadow-blue-500/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
            >
              {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 relative">
            {forgotSuccess ? (
              <div className="text-center py-4">
                ...
                <button onClick={() => setShowForgotPassword(false)} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
                  Back to Login
                </button>
              </div>
            ) : (
              <>
                ...
                <div className="flex gap-3 justify-end">
                  <button onClick={() => setShowForgotPassword(false)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg text-sm font-medium">
                    Cancel
                  </button>
                  <button
                    onClick={handleForgotPassword}
                    disabled={forgotLoading || !forgotEmail}
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed flex items-center gap-2"
                  >
                    {forgotLoading ? (
