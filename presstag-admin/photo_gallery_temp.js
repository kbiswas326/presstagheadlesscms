                ...
                <div>
                  ...
                  {!featuredImage ? (
                          ...
                          <div className="flex flex-wrap gap-2 justify-center">
                            {images.map((img, idx) => (
                              <button key={idx} onClick={() => setFeaturedImage(img)} className="border rounded-lg overflow-hidden w-20 h-14 flex items-center justify-center bg-gray-50 hover:ring-2 ring-emerald-400">
                                <img src={img.url && !img.url.startsWith('http') ? `${process.env.NEXT_PUBLIC_API_URL.replace('/api', '')}${img.url}` : img.url} alt={img.altText || 'gallery'} className="object-cover w-full h-full" />
                              </button>
                            ))}
                          </div>
                        </div>
                    ...
                    <div className="relative rounded-xl overflow-hidden border mb-4">
                      <img src={featuredImage.url && !featuredImage.url.startsWith('http') ? `${process.env.NEXT_PUBLIC_API_URL.replace('/api', '')}${featuredImage.url}` : featuredImage.url} alt={featuredImage.altText || 'Featured'} className="w-full h-48 object-cover bg-gray-50" />
                ...
                <div>
                  ...
                  <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-4">
                    {images.map((img, idx) => (
                      ...
                      >
                        <img src={img.url && !img.url.startsWith('http') ? `${process.env.NEXT_PUBLIC_API_URL.replace('/api', '')}${img.url}` : img.url} alt={img.altText || 'gallery'} className="w-full h-48 object-cover rounded-lg mb-2" />
                        <textarea
