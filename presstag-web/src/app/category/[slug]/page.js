...
async function getCategoryPosts(slug, page = 1) {
  ...
  const limit = 20;
  try {
    // API ignores limit for categories, returning all posts. We handle pagination client-side.
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/posts?category=${slug}`, {
      cache: 'no-store'
    });
    if (!res.ok) {
